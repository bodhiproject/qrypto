...
export default class SendStore {
  ...
  public init = () => {
    ...
    chrome.runtime.sendMessage({ type: MESSAGE_TYPE.GET_QRC_TOKEN_LIST }, (response: any) => {
      ...
      this.tokens[0].balance = this.app.sessionStore.loggedInAccount && this.app.sessionStore.loggedInAccount.info
        ? this.app.sessionStore.loggedInAccount.info.balance : undefined;
      this.token = this.tokens[0];
      this.maxAmount = this.token!.balance;
    });
    this.senderAddress = this.app.sessionStore.loggedInAccount && this.app.sessionStore.loggedInAccount.info
      ? this.app.sessionStore.loggedInAccount.info.addrStr : undefined;
  }
